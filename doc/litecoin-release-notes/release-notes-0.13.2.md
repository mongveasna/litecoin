TeaCoin Core version 0.13.2 is now available from:

  <https://download.teacoind.org/teacoind-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/teacoind-project/teacoind/issues>
...
updates will be released anymore. Without security updates, using a teacoind
...
ECDSA signatures inside TeaCoin transactions now use validation using
...
Previous versions of TeaCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of TeaCoin Core was to raise the default minimum
relay fee.

TeaCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
TeaCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running teacoindd without having to do any manual configuration.
...
TeaCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. TeaCoin Core will relay transactions with
...
entirely in TeaCoin Core 0.13, and it is at this time undecided whether the
...
TeaCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
TeaCoin Core automatically creates a hidden service to listen on, without
manual configuration. TeaCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if TeaCoin Core is listening, and
...
TeaCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals TeaCoin
Core to use floating fees. By default, TeaCoin Core will use floating
fees.
...
(default: `0.0002` TEA/kB).

At all times, TeaCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) TEA.
Furthermore, TeaCoin Core will never create transactions paying less than
...
`teacoind.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /teacoindrpc>
        ProxyPass http://127.0.0.1:10333/
        ProxyPassReverse http://127.0.0.1:10333/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass teacoindd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< teacoindrpc:<password>
    </Location>
...
Restarting teacoindd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in teacoind.conf
...
teacoind-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/teacoind-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The TeaCoin Core code base has
...
- `teacoind-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `teacoind-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the TeaCoin scripting system that in combination with
...
TeaCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of TeaCoin Core.
...
The two phases are distinct in the TeaCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming TeaCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for TeaCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each TeaCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of teacoinds they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of teacoinds being spent is exactly
...
- **Increased security for multisig:** TeaCoin addresses (both P2PKH addresses
  ...
  believed to be provided by TeaCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of TeaCoin's current ledger (called the UTXO set),
  ...
  note that TeaCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  TeaCoin users to individually opt-in to almost any change in the TeaCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and TeaCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the TeaCoin
client to TeaCoin Core 0.13.2 to signal for segwit support. When a
...
Since TeaCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `teacoind-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `teacoind-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
